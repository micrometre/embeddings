...
author: "Henok Wehibe"
tags: ["ai", "transformers.js", "javascript", "tutorial", "machine-learning"]
featured: true
...
What if your quiz could understand *meaning* instead of just matching exact text? Imagine a system where "neural networks learn from errors" and "NNs adjust weights based on mistakes" both get full credit — because they mean the same thing.

In this article, I'll show you how I built an interactive ML quiz with **semantic answer grading** using Transformers.js — all running 100% in the browser with no server required.

## The Problem with Traditional Quizzes

Traditional quiz systems are frustrating:

- ❌ **Exact match only** — "gradient descent" ≠ "Gradient Descent"
- ❌ **No paraphrasing** — Your own words don't count
- ❌ **Regex nightmares** — Maintaining answer patterns is painful
- ❌ **Limited feedback** — Just "right" or "wrong"
...
| 💰 **Zero cost** | No API calls or server infrastructure |
...
Define questions with expected answers and hints:
...
|-------------|------------|-------|
| "Learning from labeled data to predict outputs" | 87% | ✅ Excellent |
| "Training a model with examples that have correct answers" | 78% | ✅ Good |
| "Machine learning with training data" | 61% | ⚠️ Partial |
| "Neural networks" | 42% | ❌ Try Again |
...
for (const quiz of mlQuizzes) {
  const embedding = await embed(quiz);
  vectorIndex.add(embedding, { text: quiz });
}
...
This powers a search feature where users can find related ML concepts!

## Try It Yourself

I've open-sourced the complete project:

👉 **[Browser Embeddings Quiz on GitHub](https://github.com/micrometre/browser-embeddings)**
...
- 📝 **Essay grading** — Grade long-form answers semantically
- 🌍 **Multi-language** — Same embeddings work across languages
- 🎓 **Adaptive learning** — Suggest topics based on weak answers
- 💬 **Conversational quizzes** — Natural language Q&A
...
The future of educational technology isn't just about grading right or wrong — it's about understanding *how* someone answered and providing meaningful feedback.
